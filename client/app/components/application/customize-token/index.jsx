...
const CustomizeToken = class extends Component {
  ...
  async componentDidMount() {
    try {
      let contractInstance = await simpleStorage(window.web3);
  ...
  async onClickSave() {
    ...
    if (this.state.title.length < 1) {
      this.setState({
        titleError: "Please enter at least 1 character for the title",
      });
    } else {
      try {
        let contractInstance = await simpleStorage(window.web3);
  ...
  render() {
    if (this.state.redirectToTokenList) return <Redirect to={"/tokens/all"} />;
                  ...
                  <div className="columns is-mobile">
                    {range(2).map((index) => {
                      ...
                      return (
                        <div
                          key={index}
                          className="column rotate-in-center is-one-fifth-mobile is-one-fifth-tablet is-one-fifth-desktop"
                        >
                          <TokenType
                    ...
                    <div className="control">
                      <input
                        placeholder={`Name your ${
                          this.state.tokenType == 0 ? "sword" : "shield"
                        }`}
                        className="input"
